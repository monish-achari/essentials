...
def add_product(request):	
	...
	token_id = request.session['uid']
	all_offers = dict(db.child("Offers").get(token_id).val())
	all_locations = dict(db.child("Locations").get(token_id).val())
	all_subcategories = dict(db.child("ProductSubCategory").get(token_id).val())
...
def copy_product(request,uid):
	token_id= request.session['uid']
	product_dict = dict(db.child("Product").child(uid).get(token_id).val())
	add_data_to_table(db,token_id,product_dict,"Product","Product_Id","Product_Created_Date","Product_Modified_Date")
	return redirect('productlist')
...
def invoice_print(request,order_id,sub_id):
	# "http://localhost:8000/invoice/112765874754186604376/Sj6dkHJ9/bill.pdf"
	token_id = request.session['uid']
	ordered_products = db.child("User-Order-History").child(order_id).child(sub_id).get(token_id).val()
	if ordered_products == None:
		ordered_products = dict(db.child("User-Order-History").child(order_id).child(sub_id).get(token_id).val())
	...
	for prod in products:
		products_list.append({"name":prod.get('product_Name'),
								"mrp":prod.get("product_MRP"),
								"qty":prod.get('prodctQuantityUnit'),
								"sp":prod.get("product_SP"),
								"amount":float(prod.get("product_SP")) * float(prod.get('prodctQuantityUnit'))
								})
	...
	template_data = {
		...
		"home":address.get('addressNickName'),
		"houseno":address.get('houseNo'),
		"society":address.get('societyName'),
		...
		"pd":"{:.2f}".format(ordered_products.get('normalDiscount')),
		"subtotal":int(ordered_products.get('subtotal')),
		"total_sum":"{:.2f}".format(total_sum),
		"total_sum_round_off":"{:.2f}".format(ordered_products.get('subtotal') - int(ordered_products.get('subtotal'))),
		"item_count":len(products_list),
		"saved":"{:.2f}".format(ordered_products.get('totalMrp') - ordered_products.get('subtotal'))
